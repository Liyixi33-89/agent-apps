...
const PipelinesPage = () => {
  ...
  const modelTypeColor = (type: 'text' | 'vision') =>
    type === 'vision' ? 'bg-violet-600/20 text-violet-400 border-violet-600/30' : 'bg-sky-600/20 text-sky-400 border-sky-600/30';
              ...
              >
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-2 ml-6">
                    <span className="badge bg-gray-800 text-gray-500 text-xs">{pipeline.steps.length} steps</span>
                    <span className="text-xs text-gray-600">{new Date(pipeline.createdAt).toLocaleDateString()}</span>
                  </div>
                </div>
                      ...
                      .map((step, i) => (
                        <div key={step.key} className="flex items-start gap-3">
                          <div className="flex flex-col items-center">
                            <div className="w-7 h-7 rounded-full bg-violet-600/20 border border-violet-600/30 flex items-center justify-center text-xs text-violet-400 font-bold flex-shrink-0">
                              {i + 1}
                            </div>
