...
const PreviewModal = ({ item, lang, onUse, onClose }: PreviewModalProps) => {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="w-full max-w-5xl h-[90vh] bg-gray-900 border border-gray-700/60 rounded-2xl shadow-2xl flex flex-col overflow-hidden">
        {/* 头部 */}
        <div className="flex items-center justify-between px-5 py-3.5 border-b border-gray-800 flex-shrink-0">
          <div className="flex items-center gap-3 min-w-0">
            ...
            {item.tags.slice(0, 2).map((tag) => (
              <span key={tag} className="hidden sm:flex items-center gap-0.5 text-[10px] text-gray-500 bg-gray-800 px-1.5 py-0.5 rounded-full">
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            ...
            <div className="flex items-center gap-0.5 bg-gray-800 rounded-lg p-0.5">
              <button
                className={`p-1.5 rounded-md transition-all ${!isMobile ? 'bg-gray-700 text-white' : 'text-gray-500 hover:text-gray-300'}`}
            ...
            <button
              className="p-1.5 text-gray-500 hover:text-sky-400 rounded-lg hover:bg-sky-500/10 transition-colors"
...
const TemplateCard = ({ item, lang, onPreview, onUse }: TemplateCardProps) => (
  <div className="group rounded-2xl border border-gray-800/60 hover:border-gray-700 bg-gray-900/40 hover:bg-gray-900/80 transition-all overflow-hidden flex flex-col">
    {/* 缩略图 */}
    <div
      className="relative h-40 bg-gray-900 cursor-pointer overflow-hidden flex-shrink-0"
        ...
        {item.tags.length > 0 && (
          <div className="flex items-center gap-1 mt-2 flex-wrap">
            {item.tags.slice(0, 3).map((tag) => (
              <span key={tag} className="flex items-center gap-0.5 text-[10px] text-gray-600 bg-gray-800/60 px-1.5 py-0.5 rounded-full">
                <Tag className="w-2.5 h-2.5" />
                {tag}
              </span>
            ))}
...
const TemplateMarketPage = () => {
  ...
  return (
    <div className="flex flex-col h-full bg-gray-950">
      {/* 顶部导航栏 */}
      <div className="flex items-center gap-4 px-6 py-4 border-b border-gray-800 flex-shrink-0 bg-gray-950">
        <button
        ...
        <div className="ml-auto">
          <button
            className="flex items-center gap-1.5 text-xs text-gray-500 hover:text-white transition-colors p-1.5 rounded-lg hover:bg-gray-800"
      ...
      {/* 搜索 + 分类栏 */}
      <div className="px-6 py-3 border-b border-gray-800/60 flex-shrink-0 flex items-center gap-4 flex-wrap">
        {/* 搜索框 */}
        <div className="flex items-center gap-2 bg-gray-900 rounded-xl px-3 py-2 border border-gray-700/40 focus-within:border-gray-600 transition-colors w-64">
      ...
      <div className="flex-1 overflow-y-auto px-6 py-5">
        {loading ? (
          <div className="flex flex-col items-center justify-center h-full gap-3">
            ...
            <p className="text-sm text-gray-600">{lang === 'zh' ? '加载中...' : 'Loading...'}</p>
          </div>
        ) : filtered.length === 0 ? (
      ...
      {previewLoading && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center">
          <div className="flex flex-col items-center gap-3">
            ...
            <p className="text-sm text-gray-400">{lang === 'zh' ? '加载模板...' : 'Loading template...'}</p>
          </div>
        </div>
      )}
