...
2. **技术债务检查不可跳过**：第 0.5 步的遗留债务检查是强制步骤，不受"跳过步骤"参数影响
3. **KB 更新不可跳过**：`doc-code-to-kb` 步骤是强制步骤，不允许标记为 `⏭️ 跳过`。如果增量更新耗时过长，至少更新索引文件（第二层）
4. **反馈闭环最多 3 轮**：超过 3 轮未通过，暂停 Pipeline 等待人工介入
5. **状态文件必须实时更新**：每个步骤完成后立即写入状态文件
6. **并行步骤的依赖检查**：并行组内的步骤无依赖，但并行组之间有依赖
