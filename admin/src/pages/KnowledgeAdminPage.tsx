        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60">
          <div className="bg-gray-900 border border-gray-700 rounded-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between p-4 border-b border-gray-800">
              <h2 className="font-semibold text-gray-100">新建知识库条目</h2>
            ...
            <form onSubmit={handleCreate} className="p-4 space-y-4">
              <div>
                <label className="text-xs text-gray-400 mb-1 block">标题（中文）*</label>
                <input className="input" value={form.titleZh} onChange={(e) => setForm({ ...form, titleZh: e.target.value })} required aria-label="中文标题" />
              </div>
              <div>
                <label className="text-xs text-gray-400 mb-1 block">标题（英文）</label>
              ...
              <div className="flex items-center gap-2">
                <input type="checkbox" id="translate" checked={form.translate} onChange={(e) => setForm({ ...form, translate: e.target.checked })} className="rounded" />
                <label htmlFor="translate" className="text-sm text-gray-400">自动翻译（中英互译）</label>
              </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-800">
                <th className="text-left px-4 py-3 text-xs text-gray-500 font-medium">标题</th>
                ...
                <th className="text-left px-4 py-3 text-xs text-gray-500 font-medium">块数</th>
              ...
              {loading ? (
                ...
                <tr key={item._id} className="table-row">
                  <td className="px-4 py-3">
                    <div className="text-sm text-gray-200 font-medium">{item.title.zh || item.title.en}</div>
                    <div className="text-xs text-gray-600 truncate max-w-48">{item.title.en}</div>
                  </td>
                  <td className="px-4 py-3"><span className="badge bg-gray-800 text-gray-400">{item.sourceType}</span></td>
                  <td className="px-4 py-3"><span className="text-xs text-gray-500">{item.categoryKey || '-'}</span></td>
                  <td className="px-4 py-3"><span className="text-xs text-gray-500">{item.stats.chunkCount}</span></td>
                  <td className="px-4 py-3"><span className="text-xs text-gray-600">{new Date(item.createdAt).toLocaleDateString()}</span></td>
